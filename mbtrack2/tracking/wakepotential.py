...
class WakePotential(Element):
    ...
    def prepare_wakefunction(self, wake_type, tau, save_data=True):
        ...
        if (tau0[0] > tau[0]) or (tau0[-1] < tau[-1]):
            n = max(
                int(np.ceil((tau0[0] - tau[0]) / dtau0)),
    ...
    def track(self, bunch):
        ...
        for wake_type in self.types:
            ...
            if wake_type == "Wlong":
                ...
                bunch["xp"] += bunch[
                    "x"] * Wp_interp * bunch.charge / self.ring.E0
            elif wake_type == "Wyquad":
                bunch["yp"] += bunch[
                    "y"] * Wp_interp * bunch.charge / self.ring.E0
...
class LongRangeResistiveWall(Element):
    ...
    def update_tables(self, beam):
        ...
        if beam.mpi_switch:
            ...
            # negative sign as when bunch 0 is tracked, the others are not yet
            # passed
            self.tau[:,
                     0] = beam.mpi.mean_all[:,
            ...
            # negative sign as when bunch 0 is tracked, the others are not yet
            # passed
            self.tau[:, 0] = (mean_all[4, beam.filling_pattern] -
                              beam.bunch_index * self.ring.T1)
        ...
        for j in range(self.nt):
            for i in range(self.nb):
                ...
                if wake_type == "Wlong":
                    ...
                    sum_kick += (self.Wdip(deltaT, "x") * self.charge[i, j] *
                                 self.x[i, j])
                elif wake_type == "Wydip":
                    sum_kick += (self.Wdip(deltaT, "y") * self.charge[i, j] *
                                 self.y[i, j])
                elif wake_type == "Wxquad":
    ...
    def track(self, beam):
        ...
        if beam.mpi_switch:
            rank = beam.mpi.rank
            bunch_index = (beam.mpi.bunch_num
