...
def plot_bunchdata(filenames,
    ...
    for i, filename in enumerate(filenames):
        ...
        if dataset == "current":
            ...
            if dimension == "x":
                label = "hor. emittance (m.rad)"
            elif dimension == "y":
                label = "ver. emittance (m.rad)"
            elif dimension == "s":
                label = "long. emittance (s)"
...
def plot_phasespacedata(
    ...
    fig = sns.jointplot(
        x=x_axis * scale[option_dict[x_var]],
...
def plot_profiledata(
    ...
    if profile_plot is True:
        ...
        for i in range(num + 1):
            ax.plot(
                x_var[i] * scale[dimension_dict[dimension]],
