    ...
    def write(self):
        ...
        for dim in self.dimensions:
            self.file[self.group_name][dim][
                ...
                self.write_count * self.buffer_size:(self.write_count + 1) *
                self.buffer_size,
            ] = self.__getattribute__(dim)
            self.file[self.group_name][dim + "_bin"][
                :,
                self.write_count * self.buffer_size:(self.write_count + 1) *
                self.buffer_size,
            ] = self.__getattribute__(dim + "_bin")
    ...
    def write(self):
        ...
        for dim in self.wake_types:
            self.file[self.group_name]["tau_" + dim][
                ...
                self.write_count * self.buffer_size:(self.write_count + 1) *
                self.buffer_size,
            ] = self.__getattribute__("tau_" + dim)
            self.file[self.group_name]["profile_" + dim][
                :,
                self.write_count * self.buffer_size:(self.write_count + 1) *
                self.buffer_size,
            ] = self.__getattribute__("profile_" + dim)
            self.file[self.group_name][dim][
                :,
                self.write_count * self.buffer_size:(self.write_count + 1) *
                self.buffer_size,
            ...
            if dim == "Wxdip" or dim == "Wydip":
                self.file[self.group_name]["dipole_" + dim][
                    :,
                    self.write_count *
                    self.buffer_size:(self.write_count + 1) * self.buffer_size,
                ] = self.__getattribute__("dipole_" + dim)
        ...
        for key, value in self.track_dict.items():
            ...
            self.coherent[self.store_dict[key], :,
                          self.buffer_count] = (self.get_coherent_spectrum(
                              self.mean[value]))
            if self.higher_orders:
                self.coherent_q[self.store_dict[key], :, self.buffer_count] = (
                    self.get_coherent_spectrum(self.std[value]))
                self.coherent_s[self.store_dict[key], :, self.buffer_count] = (
                    self.get_coherent_spectrum(self.skew[value]))
                self.coherent_o[self.store_dict[key], :, self.buffer_count] = (
                    self.get_coherent_spectrum(self.kurtosis[value]))
