...
class Bunch:
    ...
    def __repr__(self):
        ...
        rep = pd.DataFrame(
            np.array([self[l] for l in self.__iter__()]).T,
    ...
    def emit(self):
        ...
        if len(self) > 1:
            ...
            if (np.array(self.ring.optics.local_dispersion)
                ...
                sig22 = (cov_x[1, 1] -
                         cov_xpdelta[0, 1] * cov_xpdelta[0, 1] / cov_z[1, 1])
                ...
                sig22 = (cov_y[1, 1] -
                         cov_ypdelta[0, 1] * cov_ypdelta[0, 1] / cov_z[1, 1])
                with np.errstate(invalid="raise"):
                    try:
    ...
    def plot_phasespace(self, x_var="tau", y_var="delta", kind="scatter"):
        ...
        fig = sns.jointplot(
            x=self.particles[x_var] * scale[x_var],
...
class Beam:
    ...
    def bunch_index(self):
        """Return an array with the positions of the non-empty bunches."""
        return np.where(self.filling_pattern)[0]
